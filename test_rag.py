...
def process_and_save_multimodal(pdf_files, image_files):
    print(
        f"Processing {len(pdf_files or [])} PDFs and {len(image_files or [])} images..."
    )
...
def query_rag(query_text: str):
    if not database_exists():
        return (
            "No database found. Please upload and process files first.",
            "No database found.",
        )
    ...
    try:
        ...
        for doc in results:
            ...
            prefix = (
                "[IMAGE DESCRIPTION"
                if doc.metadata.get("content_type") == "image_caption"
        ...
        sources = [
            f"{doc.metadata.get('source', 'Unknown')} ({doc.metadata.get('type', 'text')})"
            for doc in results
        ]
        return (
...
def query_rag_with_images(query_text: str):
    ...
    try:
        ...
        image_results = [
            ...
            or doc.metadata.get("content_type") == "image_caption"
        ]
